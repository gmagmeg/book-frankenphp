...
## Octaneの役割
**Octaneの導入**
LaravelでWorkerモードの利点を活かすうえで重要なのがOctaneです。Octaneは、Laravelアプリケーションを一度起動し、その状態をメモリに保持したままリクエストを処理する仕組みを提供します。導入は他のLaravelパッケージと同様に、ComposerコマンドとArtisanコマンドで行えます。
