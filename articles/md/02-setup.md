...
`git clone https://github.com/gmagmeg/book-frankenphp-docker`
...
本書では `.env` で `APP_PORT=8100` を設定して進めるため、実際の待受ポートは `8100` になります（未設定では8000番が使われます）。

`frankenphp` ディレクトリで、次のコマンドを実行します。
```bash
