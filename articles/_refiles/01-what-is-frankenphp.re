...
FrankenPHPは、Go言語製Webサーバー Caddy にPHPランタイムを統合したPHPアプリケーションサーバーです。作者はKévin Dunglas、初回安定版のリリースは2024年1月で、まだまだ若いOSSです。@<br>{}公式リポジトリは @<tt>{https://github.com/php/frankenphp} で、GitHubのURLをよく見るとわかるのですが、PHPの公式リポジトリとして扱われています。2025年5月にはPHPを支援する非営利組織 The PHP Foundationからの公式サポートが公表され、今日ではPHP実行環境の有力な選択肢の1つとして注目されています。@<br>{}出典: https://thephp.foundation/blog/2025/05/15/frankenphp/
...
@<b>{Workerモードによるパフォーマンス改善を狙う} Workerモードを使うと、アプリケーションの初期化プロセスを起動時に一度だけ行い、リクエスト間で再利用しやすくなります。Symfony、Laravelなどのフレームワークは便利な反面、起動時に設定ファイルなど多くの処理を読み込むため、起動コストが発生します。PHP-FPM構成ではリクエストごとに起動コストが発生しますが、FrankenPHPではこのコストを削減できるので、全体的なスループット向上が見込めます。
...
@<b>{Apache + mod_php, Nginx + PHP-FPM}前述の通りなので、ここでは省きます。
