...
FrankenPHPは、Go言語製Webサーバー Caddy にPHPランタイムを統合したPHPアプリケーションサーバーです。作者はKévin Dunglas、初回安定版のリリースは2024年1月で、まだまだ若いOSSです。公式リポジトリは @<tt>{https://github.com/php/frankenphp} で、GitHubのURLをよく見るとわかるのですが、PHPの公式リポジトリとして扱われています。2025年5月にはPHPを支援する非営利組織 The PHP Foundationからの公式サポートが公表され、今日ではPHP実行環境の有力な選択肢の1つとして注目されています。出典: https://thephp.foundation/blog/2025/05/15/frankenphp/
...
@<b>{インフラ構成をシンプルに保ちたい場合} WebサーバーとPHP実行環境を1プロセスにまとめられるため、@<tt>{Nginx} と @<tt>{PHP-FPM} の2層構成と比べてコンテナ管理が簡潔です。小規模プロダクトや個人開発など、インフラの管理コストを抑えたい場面に向いています。

@<b>{Workerモードでパフォーマンスを上げたい場合} Workerモードを使うと、アプリケーションの初期化をサーバー起動時の1回に集約できます。LaravelなどのフレームワークはPHP-FPM構成だとリクエストごとに初期化コストが発生しますが、FrankenPHPではこのコストを削減できるため、全体的なスループット向上が見込めます。

@<b>{SSEや103 Early Hintsを使いたい場合} Mercureプロトコルを介したServer-Sent Events（SSE）や、HTTP 103 Early Hintsをネイティブでサポートしています。追加モジュールなしでリアルタイム通知やパフォーマンス最適化を実現できます。

== 他の選択肢との比較

PHPのモダンな実行環境として、FrankenPHPのほかに @<b>{Swoole} や @<b>{RoadRunner} があります。
...
既存コードへの影響	@<dtp>{table align=center}低	@<dtp>{table align=center}低	@<dtp>{table align=center}高	@<dtp>{table align=center}低	@<dtp>{table align=center}中
...
@<b>{RoadRunner} Go製のアプリケーションサーバーで、PHP-FPMを置き換えるように動作します。Workerプロセスを常駐させてリクエストを処理する点はFrankenPHPのWorkerモードと思想が近く、@<tt>{Laravel Octane} からも選択できます。@<tt>{Nginx} などWebサーバーアプリケーションと組み合わせて使われることが多いです。

@<b>{FrankenPHP}上記に対して、FrankenPHPは、WebサーバーとPHP実行環境の一体化、CaddyによるHTTP/2・HTTP/3・自動TLSへのネイティブ対応、Mercure統合によるSSEサポートを組み合わせている点が独自の強みです。内部の仕組みがシンプルというよりは、利用者にとって手軽に使える（イージーな）構成になっています。Laravelとの組み合わせでは、Laravel Octaneの公式ドライバとしても採用されており、既存のLaravel開発者が試しやすい環境が整っています。
