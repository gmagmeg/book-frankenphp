 ...
 * @<b>{スレッド} 1つのプロセス内で並行して動く実行単位です。同じプロセスのメモリ空間を共有するため、起動コストは比較的低い一方で、共有状態の扱いには注意が必要です。
...
Workerモードの設定をしない場合、FrankenPHPは classic mode で動作します。このモードは従来のPHP実行環境に近い使い方ができるモードで、リクエストごとにPHPファイルを直接実行する形式をとります。内部的にスレッドプールを用いてリクエストを処理することはWorkerモードと共通しており、単純にシングルスレッドで動くわけではありません（筆者はここの棲み分けで随分悩みました）。大きな違いとして、classic mode ではリクエストごとにアプリケーションの初期化が発生します。Laravelのようなフレームワークを利用する場合は、設定やサービスコンテナの読み込みを毎回行うため、この初期化コストが性能に影響してきます。

=== Workerモード

Workerモードは、アプリケーションを一度起動し、その状態をメモリに保持したままリクエストを処理するモードです。OPcacheがPHPコードを効率よく再利用する仕組みであるのに対し、Workerモードは起動済みのアプリケーション全体を使い回せる点に特徴があります。毎回アプリケーションを起動し直さずに済むため、設定配列や設定オブジェクト、DIコンテナ、テンプレートエンジンなどなどのフレームワークの初期化コストを削減できます。このようにWorkerモードの本質は起動済みのアプリケーションを使い回せることにあり、高速＆高パフォーマンスを支える重要な技術となっています。
...
@<b>{表：classic mode と Workerモードの比較}
...
Laravel Octaneでは、@<tt>{--workers} によってリクエストを処理するワーカー数を指定できます。本書では、この構成を使って起動済みのLaravelアプリケーションを活かしながらリクエストを処理していきます。

== Octaneの役割

@<b>{Octaneの導入} FrankenPHPのWorkerモードをLaravelで活かすうえで重要なのがOctaneです。Octaneは、Laravelアプリケーションを一度起動し、その起動状態をメモリに保持したままリクエストを処理する仕組みを提供します。導入は他のLaravelパッケージと同様に、ComposerコマンドとArtisanコマンドで行えます。
...
この仕組みを実現するために、OctaneはLaravelの標準エントリーポイントである @<tt>{public/index.php} をWorkerスクリプトに差し替えます。Workerスクリプトはサーバー起動時に一度だけ実行され、Laravelアプリケーションのインスタンスを @<tt>{$app} に保持します。以降のリクエスト処理では、この @<tt>{$app} を使い回すことで、アプリケーションの再起動なしにリクエストを処理できます。

@<tt>{php artisan octane:install --server=frankenphp} を実行すると、次のようなWorkerスクリプトが @<tt>{public/frankenphp-worker.php} に生成されます。
