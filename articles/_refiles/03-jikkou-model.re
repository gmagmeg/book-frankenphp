...
@<b>{Octane導入後の仕組み}ここからはWorkerスクリプトの実装を追って、起動状態をどのように保持するかを見ていきます。@<br>{}Octaneを導入したLaravelアプリケーションは、サーバー起動時に一度だけ初期化され、その後は起動済みのアプリケーションがメモリに保持されたまま、受け取ったリクエストを順番に処理していきます。差し替えられたWorkerスクリプトはサーバー起動時に一度だけ実行され、Laravelアプリケーションのインスタンスを @<tt>{$app} に保持します。以降のリクエスト処理では、この @<tt>{$app} を使い回すことで、アプリケーションの再起動なしにリクエストを処理できます。
