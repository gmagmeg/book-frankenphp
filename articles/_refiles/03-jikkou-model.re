...
Workerモードは、アプリケーションを一度起動し、その状態をメモリに保持したままリクエストを処理するモードです。OPcacheがPHPコードを効率よく再利用する仕組みであるのに対し、Workerモードは起動済みのアプリケーション全体を使い回せる点に特徴があります。毎回アプリケーションを起動し直さずに済むため、設定配列や設定オブジェクト、DIコンテナ、テンプレートエンジンなどのフレームワーク初期化コストを削減できます。このようにWorkerモードの本質は起動済みのアプリケーションを使い回せることにあり、高速＆高パフォーマンスを支える重要な技術となっています。
